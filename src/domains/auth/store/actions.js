...
export default {
  ...
  accessToken({ state, commit }) {
    ...
    return localforage.getItem('accessToken').then(accessToken => {
      commit('setAccessToken', accessToken)
      return accessToken
    })
  },
  refreshToken({ state, commit }) {
    ...
    return localforage.getItem('refreshToken').then(refreshToken => {
      commit('setRefreshToken', refreshToken)
      return refreshToken
    })
  },
  isLogged({ dispatch }) {
    return dispatch('refreshToken').then(refreshToken => {
      if (refreshToken) {
        return refreshToken
      }
